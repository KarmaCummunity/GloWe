...
- AC16b. **Cover photo + upload performance (2026-07-27).** ✅ Camera control on the Personal Area / owner profile cover opens a cover-edit modal. Cover (and avatar) picks always downscale in-browser (`PROFILE_COVER_UPLOAD`: max edge 1600px / ~1 MB target; `PROFILE_AVATAR_UPLOAD`: 1024px) via `prepareAvatarUploadFile({ alwaysDownscale })` so phone-camera originals do not stall upload or CSS paint. Preparation runs once on file select (not again on Save); cover save patches the hero CSS variable instead of rebuilding the whole Personal Area DOM.
