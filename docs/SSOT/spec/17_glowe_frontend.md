...
**Status.** 🟡 In progress — Phase 1 (foundation) ✅ shipped 2026-09-09; Phases 2–5 per [`docs/superpowers/plans/2026-09-09-glowe-design-system-reset.md`](../../superpowers/plans/2026-09-09-glowe-design-system-reset.md). Decision `D-190`.
...
- AC4. **One shell.** ✅ Header, footer, shared `<head>` assets and the base script list live once in `partials/*.html` and are stamped into all 22 pages by `app/scripts/glowe-sync-shell.mjs` (`--check` runs in `pnpm lint`). Static header markup equals what `app.js` renders (same links, active tab, single auth CTA) so the JS pass causes no second paint. Fonts load through one non-blocking `<link>`; the never-loaded `Inter` is gone from the stack.
- AC5. **Canonical breakpoints.** ✅ guard / 🟡 migration. Only `479/639/767/1023/1279` (`max-width`) and `480/640/768/1024/1280` (`min-width`) are accepted outside `legacy.css`; mobile-first, fluid `clamp()`/grid layout, bottom tab bar `< 768px`, top nav `≥ 768px` (Phase 2).
...
- AC7. **Visual gate before merge.** ✅ `CI — GloWe E2E` `glowe-visual` job serves the PR checkout locally on `pull_request` (deployed URL only on `push`), so design PRs are validated against their own code; baselines are refreshed in the same PR.
