...
**Status.** 🟡 In progress — Phase 1 (foundation) ✅, Phase 2 (shell & layout) ✅, Phase 3 (controls & cards) ✅ and Phase 4 (pages) ✅ shipped 2026-09-09; Phase 5 (cleanup) per [`docs/superpowers/plans/2026-09-09-glowe-design-system-reset.md`](../../superpowers/plans/2026-09-09-glowe-design-system-reset.md). Decision `D-190`.
...
**Acceptance Criteria.**
- AC1. **Single token source.** ✅ `css/tokens.css` is the only stylesheet allowed to declare raw colors (`#hex`, `rgb()`, `hsl()`); it defines palette scales, semantic surfaces/text/borders, a 4px spacing grid, type scale, radii, shadows, control heights, z-index scale and motion tokens. Legacy variable names are aliases onto the new tokens until `legacy.css` is gone. Responsive and directional values are tokens too: `--container-pad` (gutter per breakpoint), `--hero-overlay` (flat light wash on phones) and `--hero-sweep` / `--hero-photo-x` (the hero photo wash flips under `html[dir=rtl]`, so no page carries its own RTL mirror).
- AC2. **Layered entry.** ✅ Pages link exactly one stylesheet, `css/glowe.css`, which declares `@layer tokens, base, legacy, components, layout, pages, utilities` and imports the layer files. A migrated component always beats its legacy rule; `legacy.css` may only shrink (line budget in `app/scripts/check-glowe-css.mjs`, TD-192). The deploy step (`glowe-minify-hash.mjs`) bundles the entry into **one** hashed CSS request.
...
- AC8. **Verification bar.** Every phase PR: `pnpm typecheck && pnpm test && pnpm lint` green; browser-verified at 390×844 / 768×1024 / 1280×900 in `en` and `he`; zero console errors introduced. Phase 5 adds Lighthouse mobile Performance ≥ 90 / Accessibility ≥ 95 on staging.
