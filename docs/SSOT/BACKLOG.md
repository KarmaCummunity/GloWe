...
| GLOWE.DS | **Design system reset** — one token layer, cascade-layered CSS entry bundled to a single request, unified `.btn`/form/feedback components, one shell stamped from partials into all pages, canonical breakpoints, lint guards, PR-time visual gate. Phase 1 (foundation) shipped; Phases 2–5 (shell/layout, controls/cards, pages, cleanup) per `docs/superpowers/plans/2026-09-09-glowe-design-system-reset.md` | agent-frontend | 🟡 In progress | `spec/17_glowe_frontend.md` FR-GLOWE-029; D-190; TD-191/TD-192 |
