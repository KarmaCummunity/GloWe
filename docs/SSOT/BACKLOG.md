...
| GLOWE.DS | **Design system reset** — one token layer, cascade-layered CSS entry bundled to a single request, unified `.btn`/form/feedback components, one shell stamped from partials into all pages, canonical breakpoints, lint guards, PR-time visual gate. Phases 1–5 (foundation, shell & layout, controls & cards, pages, cleanup & hardening) shipped 2026-09-09 per `docs/superpowers/plans/2026-09-09-glowe-design-system-reset.md`; `legacy.css` deleted, Lighthouse mobile perf 0.97 / a11y 1.0, v1.5.0 | agent-frontend | ✅ Done | `spec/17_glowe_frontend.md` FR-GLOWE-029; D-190–D-194; TD-192 ✅, TD-191/TD-194 open |
