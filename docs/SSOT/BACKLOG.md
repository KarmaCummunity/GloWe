...
| GLOWE.LAUNCH-1 | **Launch hardening Wave 1 — owner approval mode + decision notifications.** Unify volunteer-apply and event-RSVP onto one RPC that honours `registration_mode` and `capacity`; expose the open-vs-gated toggle on the volunteer-opportunity composer and add edit-after-publish; send transactional email on org approve/reject and application accept/decline; surface `org_review_note` to the rejected org. | agent-fullstack | 🟡 In progress — unified registration RPC + composer controls + capacity-aware decisions shipped (migration `0237`, D-186); remaining: edit-after-publish, transactional email, `org_review_note` to the org | `spec/17_glowe_frontend.md` FR-GLOWE-012 AC5-AC8 / FR-GLOWE-016 |
| GLOWE.LAUNCH-2 | **Launch hardening Wave 2 — performance.** Server-side `glowe_home_feed` RPC (today the home page runs six unbounded `select('*')` catalog fetches to render 10 cards); esbuild minify + content-hashed asset names + `_headers`; extract `GLOWE_TRANSLATIONS` (612 KB of the 1.07 MB `app.js`) to per-language JSON; `defer` the 20 blocking script tags; vendor `supabase-js` off the jsdelivr CDN; compress the image assets (one PNG is 2.2 MB). | agent-fullstack | ⏳ Planned | `spec/17_glowe_frontend.md`; TD-186 |
| GLOWE.LAUNCH-3 | **Launch hardening Wave 3 — launch operations.** Sentry + product analytics for GloWe web; `privacy.html`; cookie/consent banner; IS 5568 accessibility statement; meta description / `og:` tags / `robots.txt` / sitemap. | agent-fullstack | ⏳ Planned | `spec/17_glowe_frontend.md`; TD-185 |
| GLOWE.LAUNCH-4 | **Launch hardening Wave 4 — QA + rollback.** E2E for individual signup, org onboarding, event RSVP and owner accept/decline; negative E2E asserting the Wave 0 server rules hold against a direct API call; document the Cloudflare Pages rollback path. | agent-fullstack | ⏳ Planned | `RELEASE_CHECKLIST.md`; INFRA-QA-W2 |
...
| GLOWE.CARD-PARITY | **Home/Community card + comments parity** — Home compact cards render the same collapsible comment thread as Community (count tap expands); opportunity/event directory cards drop the oversized footer Share and put Share + type tag beside the ⋯ menu | agent-fe | ✅ Done | `spec/17_glowe_frontend.md` FR-GLOWE-008 AC4/AC5; FR-GLOWE-016 AC2 |
