...
| GLOWE.PERF-GATE | **Self-hosted fonts + performance regression gate.** Removed the Google Fonts `@import` from `css/styles.css` — four families (Assistant, Heebo, Noto Sans Arabic, Noto Sans Ethiopic) chained behind the stylesheet parse on a third-party origin, in front of first paint. All five families vendored as woff2 with `unicode-range` subsetting; render-path third-party origins **2 → 0**. Added `scripts/measure.mjs` byte-budget gate (flags cross-origin `@import`s specifically), `scripts/smoke.mjs` 22-page headless-browser pass, and `ci-glowe-perf.yml`. | agent-fullstack | ✅ Done | `spec/17_glowe_frontend.md` FR-GLOWE-029; D-190; `docs/superpowers/plans/2026-07-28-glowe-performance-guardrails.md` |
| GLOWE.PERF-READS | **Bounded catalog reads + indexed hot paths.** Every unbounded ordered `select` in `backend.js` now ends in `.limit()` (`LIST_HARD_LIMIT`, warns on truncation) — `listAll('comments')` previously fetched every comment in the system to render one page. Migration `0243` adds 12 missing indexes, incl. `glowe_comments (post_id, created_at)` which `glowe_home_feed` aggregates over the whole table on every home load. Measured: ~700 KB of JS (75%) ships unused per page — `app.js` alone 494 KB shipped / 378 KB dead. | agent-fullstack | 🟡 In progress (caps + indexes done; cursor pagination, `glowe_home_feed` candidate bound, and `app.js` split open) | `spec/17_glowe_frontend.md` FR-GLOWE-030; migration `0243` |
