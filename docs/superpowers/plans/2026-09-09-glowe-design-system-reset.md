...
- `glowe-visual` `footer — home @ 768` drifts above the 4 % pixel budget when run locally against either the pre- or post-split checkout (font rasterisation), while passing in CI — the baseline is environment-sensitive, not a regression.
