...
### Phase 5 — Cleanup & hardening ✅ (2026-09-09)

- [x] `legacy.css` reaches 0 lines and is deleted; guard ratchet becomes a hard ban. The last 805 lines were relocated by destination — status badges / `.muted-note` → `components/feedback.css`, form helpers → `components/forms.css`, filter accordion → `components/list-filters.css`, report box → `components/modal.css`, new `components/{section-toolbar,translate,consent-banner}.css`, photo bands → `layout-page-header.css`, `url()` artwork → `brand-art.css`, page rules → their `pages/*.css` (new `pages/{settings,home-marketing}.css`); `legacy-responsive.css`'s 8 media blocks were folded into the sheets that own them. Six permanently hidden home sections (`growth-suite`, `funding`, `engagement`, `rewards`, `how-it-works`, plus their renderers and empty data arrays) were deleted outright. `css/pages/*.css` now import into the `pages` layer; the `legacy` layer is gone from `glowe.css`.
- [x] Zero `!important` outside `utilities.css`; zero raw colors outside `tokens.css`; zero non-canonical breakpoints — `check-glowe-css.mjs` applies every rule to every sheet and hard-bans `legacy.css` / `legacy-responsive.css`. Cascade parity checked with a computed-style diff of every element on all 22 pages × guest/member × 390-he/1280-en against `staging`; the handful of intentional deltas (Google button as a plain `.btn-outline`, composer buttons no longer overriding `.btn`, onboarding type cards as flex) are design fixes.
- [x] Lighthouse (mobile, DevTools throttling, minified bundle) home: Performance 0.97, Accessibility 1.0, CLS 0.03. The LCP element on every guest page is the photo band; it is now preloaded from `<head>` (`glowe-sync-shell.mjs` derives the photo from the body class and stamps `<link rel=preload as=image media=…>` into the 18 pages with a static band) and phones get a 960px `-m.webp` cut (34–152 KB instead of 87–343 KB) — home LCP 2.3 s → 1.6 s. Shell-paint CLS: `body > main { min-height: 100vh }` keeps the footer below the fold while JS fills the page (organizations CLS 0.28 → 0.01, perf 0.83 → 0.97) and the Wishing Well ships its three stat cards statically so live numbers replace text (CLS 0.10 → 0.02). Per page (mobile, DevTools): home 0.97 / wishing-well 0.96 / organizations 0.97 / community 0.97; accessibility 1.0 everywhere except community 0.98 (empty-state `h3` outline, TD-194). axe scan (guest + member, 390/1280) clean after: `--text-subtle` darkened to a new `neutral-500` (AA on every surface), verified tick `role="img"`, footer headings `h4` → `h3`, clickable lead comments `div[role=button]` with keyboard handling.
- [x] `app/VERSION` MINOR bump `1.4.11` → `1.5.0` in this PR (the `staging` → `dev` release PR carries it).
- Bugs fixed on the way: Wishing Well summary cards read "12Open wishes" on phones/tablets (number now stacks above its label).
...
- Member-only pages whose content is JS-rendered (`my-applications`, `profile`, `saved`, `messages`, `write-post`) have no `<h1>` or jump `h2` → `h3` in Lighthouse's heading-order audit. Needs a decision on the page outline for dynamic content (a visually-hidden `h1` per page vs. promoting the first section title) — `TD-194`.
- Desktop home feed tiles are pinned to `min/max-height: 420px` (`pages/home-feed.css`) so the two-column grid stays uniform; short posts leave a visible void above the comment box. Present on `staging` before Phase 5; a design call (masonry / auto rows / smaller clamp), not a regression.
- ~~The post-merge `GloWe visual (staging)` job failed after every phase merge (all 390px header captures unstyled) while the PR-time run was green.~~ Trace evidence: the runner received the *previous* deploy's HTML (old `glowe.<hash>.css` / `glowe-version.<hash>.js`) next to the new deploy's assets during Cloudflare Pages propagation, so the old stylesheet aborted; `wait-for-url` only checked for a 200 on the root. Fixed after Phase 5: `scripts/wait-for-glowe-deploy.mjs` requires the pushed `app/VERSION` to be live and every hashed asset the HTML references to resolve with its MIME type, for 3 consecutive probes, before the journeys / visual jobs start. Second, independent cause found once the wait was in place: the visual project never pinned the backend, so on a locally served checkout (PR runs, baselines) `backend-config.js` picked local Supabase and the header CTA rendered as "Dev sign in" (105px) instead of "Sign up / Sign in" (137px) — the masked box differed from the deploy at 390px. The spec now sets `glowe-backend=dev` in its init script (same pin as the journeys, TD-193) and the eight 390px header baselines were regenerated; 32/32 against both the checkout and the live deploy.
